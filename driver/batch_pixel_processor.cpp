...
"color_replacer\n \
Matches target red green and blue pixels and replaces them with color newred, \
newgreen and newblue.\
\n\n\
See the file 'color_replace.config' to change the match colour. \
\n\n\
Output is .png images in a subfolder made in the source files \
directories. Pre-existing output will be overwritten. See 'color_replace.config' \
to change the name of the sub folder into which results will be put.\
\n\n\
...
of images to make new versions of. Images listed in an exemption file will then be \
...
used in order to prevent the overwrite of custom images. Any .txt files will be \
...
struct pixel_replacing_operation_t : public aab::visen::PixelWriteOperation
{   typedef aab::visen::Pixel Pixel;
...
void parse_config_file(std::string filename, std::string&mask_subfolder, pixel_replacing_operation_t&pixel_masking_operation)
{
    ...
    pixel_masking_operation.match_pixel.red = mask_red;
    pixel_masking_operation.match_pixel.green = mask_green;
    pixel_masking_operation.match_pixel.blue = mask_blue;
    pixel_masking_operation.replacement_pixel.red = new_red;
    pixel_masking_operation.replacement_pixel.green = new_green;
    pixel_masking_operation.replacement_pixel.blue = new_blue;
    configfile.close();
}
...
int main (int argc, char ** argv)
{
	...
	try
	{
        ...
        pixel_replacing_operation_t pixel_write_operation;
        pixel_write_operation.match_pixel.red = 255;
        pixel_write_operation.match_pixel.green = 255;
        pixel_write_operation.match_pixel.blue = 255;
        pixel_write_operation.match_pixel.alpha = 255;
        
        pixel_write_operation.replacement_pixel.red = 255;
        pixel_write_operation.replacement_pixel.green = 255;
        pixel_write_operation.replacement_pixel.blue = 255;
        pixel_write_operation.replacement_pixel.alpha = 255;
        
        
        bfile::path program_path(argv[0]);
        std::string global_config_filename (program_path.parent_path().string());
        global_config_filename += "/color_replace.config";
